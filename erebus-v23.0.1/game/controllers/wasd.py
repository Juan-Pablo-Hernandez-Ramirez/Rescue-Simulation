...
class gpsxd:
    def __init__(self, posix, posiz):
        self.posisionx = posix
        self.posisionz = posiz

    def setPosisionx(self, posix):
        self.posisionx = posix

    def setPosisionz(self, posiz):
        self.posisionz = posiz

    def getPosisionx(self):
        return self.posisionx

    def getPosisionz(self):
        return self.posisionz
...
for indice in range(0, 42, 1):
    if indice <= 9:
        ...
        archivo = '../imagenes/IMG_Blanco/C_' + str(
            indice) + '.jpg'
    image = imagen_ref(cv2.imread(archivo), enc_tipo(
        indice), "C", enc_color(indice), archivo)
...
for indice in range(0, 42, 1):
    if indice <= 9:
        ...
        archivo = '../imagenes/IMG_Blanco/I_' + str(
            indice) + '.jpg'
    image = imagen_ref(cv2.imread(archivo), enc_tipo(
        indice), "L", enc_color(indice), archivo)
...
for indice in range(0, 42, 1):
    if indice <= 9:
        ...
        archivo = '../imagenes/IMG_Blanco/D_' + str(
            indice) + '.jpg'
    image = imagen_ref(cv2.imread(archivo), enc_tipo(
        indice), "R", enc_color(indice), archivo)
...
# recuerda en que sentido se mueve el robot (F) al frente (I) izaquierda (D) derecha (U) en U
inercia = 'F'
inercia_ant = 'F'
inaux = ""
cont_ant = 0
...
def checkVic():
    ...
    print('|maximo |', max_im, '| tipo |', tip_im, '| Camara |',
          cam_enc, '| Color |', col_im, '| archivo |', archivo)
...
def lee_distancia():
    ...
    print('distancia fd ', d_fd, '  fi ', d_fi, ' derecha delantero ', d_dd,
          ' derecha central ', d_dc, " izquierda delantero ", d_id, " izquierda central ", d_ic)
    # print('logica fd ', l_fd, '  fi ', l_fi, ' derecha delantero ', l_dd,
...
def adelante():
    ...
    if (diferencia >= 50 and diferencia <= 70):
        ...
        print("sin ajuste")
    elif d_id < 100 or d_ic < 100:
        print('Me voy a alinear')
        if (diferencia < 50):
            rueda_rdf.setVelocity(speed_ordynary)
...
def ajustar():
    ...
    if (d_fd != 0):
        multiplo = d_fi / d_fd
        if (multiplo < 0.8 and d_fi > 500 and d_fd < 500 and l_ic == 1 and l_id == 1 and l_dc == 1 and l_dd == 1):
            paro()
            print("Estoy pedrido me alineo a la izquierda", multiplo)
            alinear_izquierda()
            ajustar = True
        elif (multiplo > 1.2 and d_fi > 500 and d_fd < 500 and l_ic == 1 and l_id == 1 and l_dc == 1 and l_dd == 1):
            paro()
